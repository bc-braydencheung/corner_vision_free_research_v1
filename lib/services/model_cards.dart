...
const modelCards = <ModelCard>[
  ...
  ModelCard(
    name: '分組評估（bootstrap 信賴區間）',
    purpose: '把整體比較拆成聯賽／盤口／落注距開賽時間的子集，避免優勢被平均掉或被平均出來。',
    data: '與市場基準閘門相同的已結算前瞻紀錄。',
    method:
        '每個子集逐場計算「盤口 Brier − 模型 Brier」，'
        '再以 400 次 bootstrap 重抽取 2.5%／97.5% 百分位區間。',
    gate: '子集少於 20 場不報區間；只有整個區間高於零才視為勝過盤口。',
    limits:
        '子集愈多，其中一個純靠運氣完全高於零的機會愈大（多重比較），'
  ...
  ModelCard(
    ...
    method:
        '每隊各加兩欄：距上一場的休息日數，以及開賽前 14 日內已踢場數；'
        '兩者只由該場之前的已完場賽事計算，附加在特徵尾部（30→34 欄）。',
    gate: '舊模型只讀取自己訓練時的欄數，不會因新欄而失效；未見過該隊之前的場次以聯賽預設值代入。',
    limits:
        '免費賽程未必涵蓋所有盃賽與國際賽，密集程度可能被低估；'
        '疲勞對角球的方向未經證實，加入特徵不等於預測力提升。',
  ),
  ModelCard(
    name: '結算雙軌核對',
    purpose: '兩個免費賽果來源互相核對，不一致寧可不結算，也不靜靜採用其中一個。',
    data: '馬會賽果（賽程頁讀數或賽果頁）與 football-data 免費歷史的角球數。',
    method:
        '同一場以明示對照鍵配對（聯賽、開賽日 ±1 日、隊名），'
        '兩邊都有讀數才比較；數字不同即標記為不一致。',
    gate: '不一致的場次不會結算，亦不會進入校準、市場基準閘門與模擬戶口。',
    limits:
        '只有一個來源有讀數時無從核對，仍會照該來源結算；'
        '兩個來源同時錯成同一個數字亦核對不出來。',
  ),
  ModelCard(
    name: '市場基準閘門',
    purpose: '在模型未證明勝過盤口之前，一律不出推介，只顯示觀察。',
    data: '本機前瞻紀錄：開賽前寫下的模型機率、同一條線的除水盤口機率、已結算角球數。',
    method:
        '同一批已結算場次逐場配對比較 Brier：模型 vs 除水盤口 vs 常數基本率；'
        '兩個增益都要大過自身標準誤（單邊約 95%）。',
    gate: '樣本少於 50 場、或未在統計上同時勝過盤口與常數，推介閘門保持關閉。',
    limits:
        '只以大 9.5 的機率評分，未分聯賽／盤口分層；'
        '閘門開放亦不代表未來仍然有優勢，只代表過去這批樣本上有。',
  ),
  ModelCard(
    name: '該盤口過／不過分類模型',
