  ...
  Future<void> _runMarksixBacktest() async {
    ...
    try {
      ...
      if (corrections.isNotEmpty) {
        final avgMatches = corrections
            .map((c) => c.matches)
            .reduce((a, b) => a + b) / corrections.length;
        _showMessage(
          '回測完成：${corrections.length}期，平均命中$avgMatches個號碼',
        );
      }
    } finally {
      if (mounted) setState(() => _marksixLoading = false);
          ...
          if (bootstrapProgress >= 0)
            Container(
              ...
              child: Column(children: [
                Row(children: [
                  const Icon(Icons.cloud_download, size: 18, color: Color(0xFF42E695)),
                  const SizedBox(width: 8),
                  Expanded(child: Text(
                    bootstrapProgress >= 1.0 ? '下載完成！' : '手機直接從 football-data.co.uk 下載數據',
                    style: TextStyle(fontSize: 13, color: Colors.white.withValues(alpha: 0.7)),
                  )),
                  Text('${(bootstrapProgress * 100).toStringAsFixed(0)}%',
                    style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: Color(0xFF42E695)),
                  ),
                ]),
                if (bootstrapProgress < 1.0) ...[
                  ...
                  if (bootstrapStatus.isNotEmpty) ...[
                    ...
                    Text(bootstrapStatus, style: TextStyle(
...
class _FootballUpdateCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final canTrain =
        status?.hasNewResults == true || job?.status == 'failed' || running || paused;
          ...
          if (canTrain && !running && !paused) ...[
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                label: Text(
                  job?.status == 'failed' ? '繼續上次訓練' : '重新訓練五大聯賽模型',
                ),
              ),
            ),
          ],
          ...
          if (canTrain && !running && !paused) ...[
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                icon: const Icon(Icons.model_training),
                label: Text(
                  job?.status == 'failed' ? '繼續上次訓練' : '重新訓練模型',
                ),
              ),
            ),
          ],
        ...
        children: [
          ...
          if (draws.isEmpty)
            Container(
              ...
              child: Column(children: [
                Row(children: [
                  const Icon(Icons.cloud_download, size: 18, color: Color(0xFFFFC857)),
                  const SizedBox(width: 8),
                  Expanded(child: Text(
                ...
                if (loading)
                  const SizedBox(
                    width: 24, height: 24,
                    child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFFFFC857)),
                  )
                else ...[
                  FilledButton.icon(
            ...
            Container(
              ...
              child: Row(children: [
                const Icon(Icons.cloud_done, size: 16, color: Color(0xFF42E695)),
                const SizedBox(width: 8),
                Expanded(child: Text(
                ...
                TextButton.icon(
                  ...
                  label: Text(loading ? '同步中' : '檢查更新', style: const TextStyle(fontSize: 11)),
                ),
              ]),
            ),
          ...
          if (stats != null && viewMode == 'stats') ...[
            ...
            for (final draw in draws.reversed.take(20))
              _DrawCard(draw: draw),
            if (draws.length > 20)
              Padding(
          ...
          const SizedBox(height: 20),
          _Disclaimer(
            text: '六合彩為完全隨機遊戲，所有統計及預測僅供個人研究參考，不構成投注建議。',
          ),
        ],
      ),
    );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 14, color: selected ? const Color(0xFF42E695) : Colors.white54),
...
class _StatsDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('📊 統計概覽', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
      const SizedBox(height: 12),
      // Key metrics row
      Wrap(spacing: 8, runSpacing: 8, children: [
        _StatCard(label: '總期數', value: '${stats.totalDraws}', color: const Color(0xFF42E695)),
        _StatCard(label: '奇偶比', value: stats.oddEvenRatio.toStringAsFixed(2), color: const Color(0xFF7FD1FF)),
        _StatCard(label: '平均總和', value: stats.avgSum.toStringAsFixed(0), color: const Color(0xFFB491FF)),
        _StatCard(label: '連號率', value: '${(stats.consecutiveRate * 100).toStringAsFixed(0)}%', color: const Color(0xFFFFC857)),
        _StatCard(label: '平均頭獎', value: '\$${(stats.topPrizeAvg / 10000).toStringAsFixed(0)}萬', color: const Color(0xFFFF8FA3)),
        _StatCard(label: '平均投注額', value: '\$${(stats.avgTurnover / 1000000).toStringAsFixed(0)}M', color: const Color(0xFF42E695)),
      ]),
      const SizedBox(height: 20),
      // Hot numbers
      const Text('🔥 近期熱號', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
      const SizedBox(height: 8),
      _NumberBalls(numbers: stats.hotNumbers, color: const Color(0xFFFF8FA3)),
      const SizedBox(height: 16),
      // Cold numbers
      const Text('❄️ 近期冷號', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
      const SizedBox(height: 8),
      _NumberBalls(numbers: stats.coldNumbers, color: const Color(0xFF7FD1FF)),
      const SizedBox(height: 20),
      // Frequency grid
      const Text('📈 號碼頻率分佈', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
      const SizedBox(height: 8),
      _FrequencyGrid(frequency: stats.numberFrequency),
      const SizedBox(height: 12),
      Text('日期範圍：${stats.dateRange}', style: TextStyle(
        color: Colors.white.withValues(alpha: 0.4), fontSize: 11,
      )),
    ]);
  }
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: RadialGradient(
              colors: [color.withValues(alpha: 0.5), color.withValues(alpha: 0.25)],
            ),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text(draw.drawDate, style: TextStyle(
            color: Colors.white.withValues(alpha: 0.5), fontSize: 12,
          )),
          const Spacer(),
          if (draw.totalTurnover > 0)
            _Pill(
              label: '總投注: \$${(draw.totalTurnover / 1000000).toStringAsFixed(1)}M',
              color: const Color(0xFF42E695),
            ),
        ]),
        const SizedBox(height: 12),
          ...
          ...draw.numbers.map((n) => Padding(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: const Color(0xFFFF8FA3).withValues(alpha: 0.6), width: 1.5),
              ),
              alignment: Alignment.center,
        ...
        if (draw.prizes.isNotEmpty) ...[
          ...
          Wrap(spacing: 8, runSpacing: 4, children: [
            for (final p in draw.prizes.take(4))
              Text('${p.name}: \$${_fmt(p.prizePerUnit)} (${_units(p.winningUnits)}注)',
...
class _PredictionPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('🎯 AI 預測', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
      const SizedBox(height: 12),
      Row(children: [
        Expanded(child: FilledButton.icon(
          onPressed: onGenerate, icon: const Icon(Icons.auto_awesome, size: 18),
          label: const Text('產生預測'),
        )),
        const SizedBox(width: 10),
        Expanded(child: OutlinedButton.icon(
          onPressed: onBacktest, icon: const Icon(Icons.history, size: 18),
      ...
      if (prediction != null && prediction!.recommendedNumbers.isNotEmpty) ...[
        Center(child: _Pill(
          label: '信心: ${prediction!.confidenceLabel == 'high' ? '高' : prediction!.confidenceLabel == 'medium' ? '中' : '低'} (${prediction!.confidence.toStringAsFixed(0)}%)',
        ...
        Wrap(alignment: WrapAlignment.center, spacing: 4, runSpacing: 8, children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              gradient: const RadialGradient(colors: [Color(0xFF42E695), Color(0x6642E695)]),
              border: Border.all(color: const Color(0xFF42E695).withValues(alpha: 0.7), width: 2.5),
            ),
            alignment: Alignment.center,
            child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Container(
        ...
        child: Row(children: [
          const Icon(Icons.warning_amber, size: 16, color: Color(0xFFFFC857)),
          const SizedBox(width: 8),
          Expanded(child: Text(
